...
class CustomerTransformer():
    ...
    def run_all(self, force_run=False):
        ...
        if df.empty:
            self.logger.warning("Received an empty DataFrame for Customer transformation.")
            return df
    ...
    def _read_file(self) -> pd.DataFrame:
        return pd.read_csv(self.read_path, dtype={'customer_zip_code_prefix': str})
    ...
    def _clean_column_names(self, df: pd.DataFrame) -> pd.DataFrame:
        df.columns = ['customer_id', 'customer_unique_id', 'zip_code', 'city', 'state', 'lat', 'lng']
        return df
